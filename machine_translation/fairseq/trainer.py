...
from fairseq.distributed_utils import all_reduce, MIN_OP
...
class Trainer(object):
    ...
    def __init__(self, args, task, model, criterion, dummy_batch = None):
        ...
        if args.spl:
            loss_chart = torch.zeros([ task.split_len['train'],]) + 100.0 
            self.loss_chart = loss_chart.cuda()
            self.pace = np.linspace(0.25, 1.0, 10 )
            self.lambda_t = 100.0
            self.sortN = None   
            print("SPL prepared %d length loss chart"%(task.split_len['train'])) 
    ...
    def train_step(self, samples, dummy_batch=False, assistant = None, assistant_queue = None, weights = None):
        ...
        for i, sample in enumerate(samples):
            ...
            try:
                ...
                if self.args.assistant: 
                    ...
                    losses, sample_size, logging_output, precisions = self.task.train_step(
                        sample, self.model, self.criterion, self.optimizer,
                        ignore_grad, lambda_t = self.lambda_t
                    )
                else:
                ...
                if self.args.spl and not dummy_batch:
                    y_lengths = utils.get_len(sample['target'].cpu().numpy(), self.task.target_dictionary.pad())
                    norm_losses = np.divide(losses.detach().cpu().numpy(), y_lengths)
                    self.loss_chart[sample['id'].cpu().numpy()] = torch.from_numpy(norm_losses, ).type(torch.FloatTensor).cuda()
                    if self.args.distributed_world_size > 1:
                        all_reduce( self.loss_chart, op= MIN_OP) 

                # prepare data for assistant trainning
                if assistant is not None and np.random.rand() < SEC_TRAIN_RATIO:
                    sec_batch_size = sample['id'].size(0)
                    indices_sec = np.random.choice(sample['id'].size(0), sec_batch_size)
                ...
                elif assistant_queue is not None and np.random.rand() < SEC_TRAIN_RATIO:
                    sec_batch_size = sample['id'].size(0)
                    local_indices_sec = np.random.choice(sample['id'].size(0), sec_batch_size)
                    global_indices_sec = sample['id'][local_indices_sec].cpu().numpy()                    

                    l = losses[local_indices_sec]
                    l = l.detach().cpu().numpy()
                    if not assistant_queue.full():
                        assistant_queue.put((global_indices_sec,l), block=False)
                    else:
    ...
    def valid_step(self, sample, raise_oom=False):
        ...
        # gather logging outputs from all replicas
        #print(logging_output, sample_size, force=True, flush=True)
        if self.args.distributed_world_size > 1:
            logging_output, sample_size = zip(*distributed_utils.all_gather_list(
                [logging_output, sample_size], valid=True,
            ))
