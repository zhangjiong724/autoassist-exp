...
class AssistantEpochBatchIterator(object):
    """A multi-epoch iterator over a :class:`torch.utils.data.Dataset`.
    ...
    Args:
        dataset (~torch.utils.data.Dataset): dataset from which to load the data
        collate_fn (callable): merges a list of samples to form a mini-batch
        assistant (~AssistantSampler): the Assistant instance
        max_tokens (int): 
        batch_sampler (~torch.utils.data.Sampler): an iterator over batches of
            indices
        seed (int, optional): seed for random number generator for
        ...
        shard_id (int, optional): which shard of the data iterator to
            return. Default: ``0``
        batch_method (['bins', 'sentences']) method to distribute instances
    """

    def __init__(self, dataset, collate_fn, assistant, max_tokens, max_sentences, required_batch_size_multiple, shard_num, shard_id, batch_method, seed=1):
        assert isinstance(dataset, torch.utils.data.Dataset)
        self.dataset = dataset
        self.collate_fn = collate_fn
        ...
        self._cur_epoch_itr = None
        self._next_epoch_itr = None
        self._supports_prefetch = False
        self.init_batch_sampler()

    def init_batch_sampler(self, shuffle = False):
        """ Return an assistant iterator over data """
        self.frozen_batches = self.assistant.batch_by_size(
                self.dataset.num_tokens, max_tokens=self.max_tokens, max_sentences=self.max_sentences,
                required_batch_size_multiple=self.required_batch_size_multiple, batch = self.batch_method,
                shard_num = self.shard_num, shard_id = self.shard_id, shuffle = shuffle,
            )
    ...
    def _get_iterator_for_epoch(self, epoch, shuffle, fix_batches_to_gpus=False):
        ...
        self.init_batch_sampler(shuffle = shuffle)

        self.dataset.prefetch(self.frozen_batches.indices)
        batches = self.frozen_batches
