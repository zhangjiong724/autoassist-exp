...
#SUM_OP = {'c10d': torch.distributed.ReduceOp.SUM, 'no_c10d':torch.distributed.deprecated.reduce_op.SUM}
#MIN_OP = {'c10d': torch.distributed.ReduceOp.MIN, 'no_c10d':torch.distributed.deprecated.reduce_op.MIN}

SUM_OP = {'c10d': torch.distributed.ReduceOp.SUM, 'no_c10d':torch.distributed.ReduceOp.SUM}
MIN_OP = {'c10d': torch.distributed.ReduceOp.MIN, 'no_c10d':torch.distributed.ReduceOp.MIN}
...
def all_gather_list(data, group=None, max_size=65279, valid=False):
    ...
    for i in range(world_size):
        ...
        if size > max_size:
            for ii in range(world_size):
                print("rank%d, out_buffer="%(ii), buffer[ii*max_size: (ii+1)*max_size], force=True, flush=True)
            print("enc_size=%d, buffer_rank[0]=%d, buffer_rank[1]=%d"%(enc_size, buffer_rank[0], buffer_rank[1]), force=True, flush=True)
            print("Rank %d size= 255 * %d + %d = %d"%(i, utils.item(out_buffer[0]), utils.item(out_buffer[1]), size), force=True, flush=True)
            raise ValueError('reduced data exceeds max_size: {}'.format(size))
        if size > 0:
            result.append(
